...
export default function Employees({
  ...
  const handleBulkXlsxImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (rows.length < 2) {
        setBulkStatusMsg({ text: 'فایل اکسل خالی یا بدون ردیف داده است.', type: 'error' });
        return;
      }
      // Header-based column mapping (not position-based)
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const code = (codeIdx >= 0 ? parts[codeIdx] : parts[0] || '').toUpperCase();
        const name = nameIdx >= 0 ? parts[nameIdx] : parts[1] || '';
        const unit = (unitIdx >= 0 && parts[unitIdx]) ? parts[unitIdx] : (parts[2] || 'سالن تولید');
        const roleVal = (roleIdx >= 0 && parts[roleIdx]) ? parts[roleIdx].toLowerCase() : (parts[3]?.toLowerCase() || '');
        let role: UserRole = 'employee';
        if (roleVal === 'supervisor' || roleVal === 'سرپرست') role = 'supervisor';
        else if (roleVal === 'admin' || roleVal === 'مدیر') role = 'admin';
        const username = (usernameIdx >= 0 && parts[usernameIdx]) ? parts[usernameIdx].toLowerCase() : (parts[4]?.toLowerCase() || `user_${code.toLowerCase().replace(/[^a-z0-9]/g, '')}`);

        if (!code || !name) { skippedCount++; continue; }
        if (seenCodes.has(code)) { skippedCount++; continue; } // skip duplicate in file
        ...
        if (existsCode || existsUser) { skippedCount++; continue; }

        onAddEmployee({ name, code, unit, profileId: defaultProfId, role, username });
        seenCodes.add(code);
        addedCount++;
      }
